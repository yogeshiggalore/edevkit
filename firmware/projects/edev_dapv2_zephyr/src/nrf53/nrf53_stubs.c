 ...
 *   xxd -i stub.bin   # paste the resulting byte array below + update size
 *
 * Algorithm reference: docs/NRF5340_ALGORITHMS.md §4 Stages 4–8 + §7.
 *
 * Stub version: 224 B (2026-06-30). Adds Net VMC.RAMBLOCK[n].POWERSET
 * prologue to power on all 8 Net SRAM blocks before the stub touches
 * SRAM. Without this, bench nRF5340 DK silicon enters lockup (DHCSR
 * S_LOCKUP=1) on the first SRAM store — RAM blocks default to powered-
 * off after a CTRL-AP RESET. Ring Pro 351 silicon tolerated the missing
 * VMC writes (different default state); the new prologue makes the stub
 * portable across nRF5340 variants. See
 * reference_nrfjprog_stub_disassembly_2026_06_26 — the App stub does
 * the equivalent for App VMC at 0x50081604.
 */
...
/* ------------------------------------------------------------------ */
/* Net stub binary blob — 224 B from net_disable_approtect.s          */
/* ------------------------------------------------------------------ */
const uint8_t nrf53_net_stub[] = {
	0x00, 0x10, 0x00, 0x21, 0x59, 0x00, 0x00, 0x00, 0x41, 0x00, 0x00, 0x00,
	...
	,
};
const uint16_t nrf53_net_stub_size = sizeof(nrf53_net_stub);

BUILD_ASSERT(sizeof(nrf53_net_stub) == 224,
				      ...
				      uint32_t *out_approtect)
{
	...
	/* Stage 7+8 — Poll for completion. We watch UICR.APPROTECT as the
	 * authoritative success indicator (it's the actual chip state we
	 * care about) and read the SRAM marker only for diagnostics.
	 *
	 * Why not the SRAM marker alone: empirically on bench nRF5340 DK
	 * silicon, Net CPU's stores to 0x21000000 via its local data bus
	 * don't become visible to the host via Net AHB-AP — likely an
	 * SPU/bus-arbitration asymmetry where the AHB-AP debug shortcut
	 * sees a different SRAM view than the CPU. Even though UICR.NVMC
	 * stores DO take effect (verified: UICR.APPROTECT reads 0x50FA50FA
	 * after the stub runs), the SRAM markers stay at 0x00000000.
	 *
	 * UICR.APPROTECT readback is the actual chip-state we care about
	 * post-stub. Treat it as the source of truth. */
	...
	/* Also read the marker once for the response payload. May be
	 * 0xDEADC0DE on Ring Pro 351 (visible via AHB-AP) or 0x00000000
	 * on DK silicon (CPU writes not visible) — either is fine if
	 * APPROTECT made it. */
	(void)nrf53_mem_read(NRF53_AP_NET, NRF53_CSW_NET, net_sram_marker, &marker);
	if (out_marker) { *out_marker = marker; }
	if (out_approtect) { *out_approtect = approtect; }

	if (approtect != NRF53_UNLOCK_VAL) {
		LOG_ERR("Net UICR.APPROTECT readback 0x%08x ≠ 0x50FA50FA "
			"(marker=0x%08x)", approtect, marker);
		return NRF53_STUB_FAIL;
	}
	LOG_INF("Net UICR.APPROTECT = 0x50FA50FA confirmed (marker=0x%08x)",
		marker);
